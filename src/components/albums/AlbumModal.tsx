...
const Schema = z.object({
  ...
  event_date:  z.string().regex(/^\d{4}-\d{2}-\d{2}$/, '날짜를 선택해주세요.'),
  type:        z.enum(['training', 'competition']),
})

type FormData = { title: string; description: string; event_date: string; type: 'training' | 'competition' }

const today = () => new Date().toISOString().slice(0, 10)
const EMPTY: FormData = { title: '', description: '', event_date: today(), type: 'training' }
...
export default function AlbumModal({ onClose, onSuccess }: AlbumModalProps) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      captureException(err, { action: 'create_album' })
    ...
    >
      <div className="bg-white w-full sm:max-w-lg rounded-t-2xl sm:rounded-2xl shadow-xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 sticky top-0 bg-white">
          <h2 className="font-bold text-gray-900">앨범 만들기</h2>
          <button
            onClick={onClose}
